...
function getSettingsContent(settingsFileUrl, isRemote) {
    ...
    let settingsCacheFileContent = fetch(settingsFileUrl, {});
...
function isHideRule(ruleTitle) {
    ...
    let ruleTemp = ruleTitle;
    for (let i = 0; i < settings.hideSymbols.length; i++) {
...
if (depotStatus.showTips !== false) {
    ...
    try {
        ...
        if (updateMode === 0) {
            ...
            let localDepotRule = JSON.parse(getRule());
        ...
        if (remoteDepotRule.oldVersion < remoteDepotRule.version) {
            d.push({
                ...
                url: "hiker://empty#" + JSON.stringify(remoteDepotRule) || "",
                col_type: "text_center_1",
    ...
    for (let i = 0; i < settings.authorList.length; i++) {
        ...
        let picUrl = null;
        let picUrlJS = settings.authorList[i].match(/picUrl=.[\s\S]*?'/) + ";";
        eval(picUrlJS);
        ...
        d.push({
            ...
            url: "hiker://empty#" + settings.authorList[i],
            pic_url: picUrl
        });
    }
}
